...
use callme::{
    audio::{record::OpusFramer, OPUS_STREAM_PARAMS},
    net::bind_endpoint,
...
async fn main() -> Result<()> {
    ...
    while let Some(conn) = rtc.accept().await? {
        info!("accepted");
        // let file_path = args.playback.clone();
        // let ctx = ctx.clone();
...
async fn handle_connection(
    ...
    //     .await
    //     .map_err(|e| anyhow!("{e:?}"))?;
    if let Some(filepath) = args.playback_file {
        ...
        std::thread::spawn({
            move || {
                if let Err(err) = stream_wav(filepath, sender) {
...
fn stream_wav(file_path: PathBuf, sender: broadcast::Sender<MediaFrame>) -> Result<()> {
    ...
    let mut encoder = OpusFramer::new(OPUS_STREAM_PARAMS);
    'outer: loop {
